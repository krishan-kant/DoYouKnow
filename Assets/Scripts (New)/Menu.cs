...
public class Menu : MonoBehaviour {
	...
	void Update() { //this takes care of moving all of the cards in the scroll. It resets them back to the top as well
		for(int k=0;k<6;k++) {
			float curPos = scroll2[k].transform.localPosition.y;
			if (curPos < (Screen.height-300)*-1) {
				...
				scroll2[k].transform.localPosition = new Vector2 (scroll2[k].transform.localPosition.x, curPos - 100 * Time.deltaTime);
		}
	}
}
